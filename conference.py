...
MEMCACHE_FEATURED_SPEAKER_KEY = "FEATUED_SPEAKER"
...
# - - - Speaker objects - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(SpeakerForm, SpeakerForm, 
            path='createSpeaker', http_method='POST', 
            name='createSpeaker')
...
# - - - Session objects - - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        ...
        # get conference from the websafeConferenceKey
    ...
    def getConferenceSessionsByType(self, request):
        ...
        conf = ndb.Key(urlsafe=wsck).get()
        sessions = Session.query(ancestor=conf.key)
        sessions2 = sessions.filter(Session.typeOfSession == request.typeOfSession)
        return SessionForms(items=[self._copySessionToForm(session) 
...
# - - - Wishlist - - - - - - - - - - - - - - - - - - - - - - -
    ...
    @ndb.transactional(xg=True)
    def _addToWishlist(self, request, reg=True):
        """Register or unregister user for selected conference."""
        retval = None
        prof = self._getProfileFromUser() # get user Profile
        ...
        curSession = ndb.Key(urlsafe=wssk).get()
        if not curSession:
            raise endpoints.NotFoundException(
...
# - - - Task 3 - - - - - - - - - - - - - - - - - - - - - - - 
# additional queries
# query to show conferences that are less then half full
    @endpoints.method(message_types.VoidMessage, ConferenceForms,
            path='getLowReg',
            http_method='POST', name='getLowReg')
...
# query to show conferences with no sessions
    @endpoints.method(message_types.VoidMessage, ConferenceForms,
            path='getNoSessions',
            http_method='POST', name='getNoSessions')
...
# - - - Task 3 - - - - - - - - - - - - - - - - - - - - - - - -        
# solution to querying no workshops and no sessions after 7:00 pm
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='getNoWorkshops',
            http_method='POST', name='getNoWorkshops')
    def getNoWorkshops(self, request):
        """Return sessions before 7:00 PM and no workshops."""
        # create list to store session to be returned
        sessToReturn = []
        sessions = Session.query(Session.typeOfSession != 'Workshop')
        for s in sessions:
            timeParts = s.startTime.split(':')
            # only add sessions that start before 7PM (19 hours)
            if int(timeParts[0]) < 19:
                sessToReturn += [s]
        return SessionForms(items=[self._copySessionToForm(session) 
            for session in sessToReturn])
...
# - - - Task 4 - - - - - - - - - - - - - - - - - - - - - - - - - - -
    # endpoint to retrieve featured speaker
    @endpoints.method(message_types.VoidMessage, StringMessage,
            path='featuredSpeaker',
            http_method='GET', name='getFeaturedSpeaker')
    def getFeaturedSpeaker(self, request):
        """Show featured speaker."""
